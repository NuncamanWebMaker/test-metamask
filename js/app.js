// METAMASK CONNECTION
const TIMEOUT = 1000;
const COLLECTION_NAME = 'MagiaVerde';
let editions = [];
let dots = 1;
...
function updateStatusText(isOwner, checking) {
  ...
  const imageStatus = document.querySelector('.owner-img');

  if(checking) {
    if(isOwner) {
      statusText.innerText = `You do own ${COLLECTION_NAME}!! 😻 Let's see how many${renderDots(dots)}`;
    } else {
      statusText.innerText = `Pronto podrás visualizar tus NFT de ${COLLECTION_NAME} ${renderDots(dots)}`;    }
  } else {
    if(isOwner) {
